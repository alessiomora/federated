...
class CSVMetricsManager(tf.test.TestCase):
  ...
  def test_save_metrics_returns_flat_dict_with_scalars(self):
    csv_file = os.path.join(self.get_temp_dir(), 'test_dir', 'metrics.csv')
    csv_mngr = csv_manager.CSVMetricsManager(csv_file)
    input_data_dict = _create_scalar_metrics()
    appended_data_dict = csv_mngr.save_metrics(0, input_data_dict)
    self.assertEqual(
  ...
  def test_save_metrics_returns_flat_dict_with_nonscalars(self):
    csv_file = os.path.join(self.get_temp_dir(), 'test_dir', 'metrics.csv')
    csv_mngr = csv_manager.CSVMetricsManager(csv_file)
