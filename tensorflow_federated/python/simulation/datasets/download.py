...
def _fetch_lzma_file(origin: str, filename: str):
  ...
  with urllib.request.urlopen(origin) as in_stream, \
    ...
    with tqdm.tqdm(
        total=total_size,
        desc=f'Downloading {os.path.basename(origin)}') as progbar:
...
def get_compressed_file(origin: str, cache_dir: Optional[str] = None) -> str:
  ...
  if ext != '.lzma':
    raise ValueError('Only decompressing LZMA files is supported. If the file '
                     'is LZMA compressed, please append a .lzma suffix.')
